	...
	void main() {
		if (gl_FragCoord.x * texelSize.x < RENDER_SCALE.x  && gl_FragCoord.y * texelSize.y < RENDER_SCALE.y )	{
			...
			if (iswater > 0.9) {
				...
				//color = ((shading*diffuseSun)*texelFetch2D(colortex4,ivec2(6,37),0).rgb*8./150./3.)*albedo/3.14;
				...
				vec3 rtPos = rayTraceWater(reflectedVector,fragpos.xyz,blueNoise(), fresnel);
				if (rtPos.z <1.){
				...
				reflection.rgb = mix(sky_c.rgb, reflection.rgb, reflection.a);
					...
					vec3 sunSpec = GGX(normal,-normalize(fragpos),  lightSign*sunVec, rainStrength*0.2+roughness+0.05+clamp(-lightSign*0.15,0.0,1.0), vec3(f0)) * texelFetch2D(gaux1,ivec2(6,37),0).rgb*8./3./150.0/3.1415 * (1.0-rainStrength*0.9);
				#else
				...
				gl_FragData[0].rgb =clamp((color*alpha0*(1.0-fresnel)+reflected)/gl_FragData[0].a,0.0,65100.0);
